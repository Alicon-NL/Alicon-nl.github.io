(function(global, factory) { typeof exports === "object" && typeof module !== "undefined" ? module.exports = factory() : typeof define === "function" && define.amd ? define(factory) : (global = typeof globalThis !== "undefined" ? globalThis : global || self, global.aim = factory()); }(this, function (exports) {
  ...
  function Api(url, options = {}) {
    if (!(this instanceof Api)) return new Api(...arguments);
    this.options = options;
    this.url = new URL(url || '', options.basePath || (self.document ? self.document.location.href : dmsOrigin));
  ...
  Api.prototype = {
    ...
    header(selector, context){
      if (typeof selector === 'object'){
        Object.entries(selector).forEach(entry => this.header(...entry));
      } else {
    ...
    getAccessToken(getAccessToken){
      this.options.getAccessToken = getAccessToken;
      return this;
    },
    // headers(selector, context) { // Deprecated
    ...
    fetch(){
      return new Promise(async (resolve, reject) => {
        ...
        // console.log(333, accessToken, this.url.href, this.options);
        fetch(this.url.href, this.options)
        .then(response => {
          ...
          response.text().then(content => {
            if (response.headers.get('Content-Type').includes('json')) {
              var [s,msg,data] = content.match(/(.*?)(?=\[|\{)(.*)/s
              );
              if (msg) {
                const divWarning = $(document.querySelector('div.message') || $('div').class('message').parent(document.body));
                divWarning.append(
                  $('div').text(response.url).html(msg),
                )
                console.warn(response.url, msg);
              }
        ...
        // .then(response => response.headers.get('Content-Type').includes('json') ? response
        // .json()
  ...
  Object.defineProperties(Api.prototype, {
    ...
    onprogress: { value: function(e){
      ...
      var msg = `%c${this.options.method} ${this.responseURL} ${this.status} (${this.statusText}) ${this.response.length} bytes ${new Date().valueOf() - this.startTime.valueOf()}ms`;
      if (this.elStatus){
        ...
        if (xhr.status >= 400) {
          ...
          try {
            ...
            messagePopup({
              ...
              trace: error.trace,
              body: arr[1],
              url: this.options.method.toUpperCase() + ' ' + decodeURIComponent(xhr.request.url.href),
            });
          } catch (err) {
  ...
  function Client (config = {}) {
    console.warn('Client', ...arguments);

    config = this.config = Object.assign(...arguments);
    console.log('Client', this.config);
    // this.options = options;
  ...
  Client.prototype = {
    ...
    async loadConfig() {
      config = await this.api('/aliconnect/config', {
    ...
    api(path, options) {
      const api = new Api(this.url + path, options).header({
        Accept: 'application/json',
      }).getAccessToken(getAccessToken)
      return options ? api.fetch() : api;
    }
  }
  Object.defineProperties(Client.prototype, {
    ...
    //   return aim()
    //   .url(src.match(/\/\//) ? src : this.url + src)//new URL(src, this.url).href)
  ...
  Object.assign(aim, {
    ...
    // url: (url,options) => new Request(...arguments),
    urlToId,
    idToUrl,
    extend,
    fetch(path, options) {
      const api = new Api(path, options);
      return options ? api.fetch() : api;
    },
