...
AS
	SELECT --top 1
		convert(varchar(20),B.boekingId) id
		,BD.bedrijf
		,P.bedrijf_id
		,CONVERT(date,B.datum) as datum
		,CASE WHEN DATEPART(year,B.datum) = 2013 THEN '1-1-2014' ELSE datum END AS boekdatum
		,CASE WHEN DATEPART(year,B.datum) = 2013 THEN 2014 ELSE DATEPART(year,datum) END AS jaar
		,CASE WHEN DATEPART(year,B.datum) = 2013 THEN 1 ELSE DATEPART(Q,datum) END AS q
		,ROUND(bedrag,2)bedrag
		,ROUND(excl,2)excl
		,ROUND(bedrag-excl,2) as btw
		,P.postId
		,P.boekNr
		,P.Omschrijving boekOms
		,ISNULL(B.kasgiro,'') kas 
		,B.btwprocent
	...
	AND BD.bedrijf IN ('MJVK Beheer BV','Alicon Projects BV','Alicon Systems BV')
	--AND BD.bedrijf IN ('MJVK Beheer BV')
	--where boeknr not in (2212,2211,2221,2222,2224,2111,2252,2254,2255)
	--where boeknr not in (2111,2211,2212,2221,2222,2224,2252,2254,2255)
	--where boeknr not in (2254) AND B.PostID IN (474200,474300,481000)
	--where B.PostID NOT IN (474200,474300,481000)
	--where B.PostID NOT IN (474200) AND KasGiro='B'
	--WHERE KasGiro='B'
	--where boeknr not in (0000)
	--AND boeknr IN (1251)
	--- WHERE B.datum<'01-01-2018'-- OR BoekingID=3709
GO

ALTER VIEW [b17].[boekingbank]
AS
	SELECT 
		B.bankId,BB.boekingId AS id,BB.bedrag boekBedrag,ISNULL(BB.bedrag,B.AfBijBedrag) bedrag,B.datum,DATEPART(year,B.datum) jaar,BD.bedrijf,Mededelingen
	FROM
		dbo.bank B 
		LEFT OUTER JOIN gb.boekingbank BB ON BB.bankId=B.bankid
...
AS
	...
	INSERT @TB SELECT id,datum,NULL,bedrijf,Excl,boekOms,Omschrijving,Relatie,boekNr 
	FROM b17.boeking R WHERE jaar = @jaar AND boeknr>2000

	INSERT @TB SELECT null,'31-12-'+@jaar,NULL,bedrijf,SUM(ROUND(Excl*0.8,2)),'WINST','WINST','',1224 
	FROM b17.boeking R WHERE jaar = @jaar AND boeknr>2000 GROUP BY bedrijf
	INSERT @TB SELECT null,'31-12-'+@jaar,NULL,bedrijf,-SUM(ROUND(Excl*0.2,2)),'VB','VB','',3001 
	FROM b17.boeking R WHERE jaar = @jaar AND boeknr>2000 GROUP BY bedrijf
	...
	INSERT @TB SELECT null,'31-12-'+@jaar,NULL,bedrijf			,-SUM(ROUND(Excl*0.2,2)),'VB','VB','',1172 
	FROM b17.boeking R WHERE jaar = @jaar AND boeknr>2000 GROUP BY bedrijf
	INSERT @TB SELECT null,'31-12-'+@jaar,NULL,'MJVK Beheer BV'	, SUM(ROUND(Excl*0.2,2)),'VB','VB','',1172 
	FROM b17.boeking R WHERE jaar = @jaar AND boeknr>2000 GROUP BY bedrijf
	--UPDATE gb.boeking SET bedrag=B.Bedrag,excl=B.bedrag FROM ( 
	UPDATE gb.boeking SET bedrag=B.Bedrag,excl=B.bedrag FROM ( 
		SELECT -SUM(ROUND(Excl*0.2,2)) Bedrag FROM b17.boeking R WHERE jaar = @jaar AND boeknr>2000 
	...
	INSERT @TB SELECT null,'31-12-'+@jaar,NULL,bedrijf			,-SUM(BTW),'BTW','BTW','',1172 
	FROM b17.boeking R WHERE jaar = @jaar AND boeknr>2000 AND BTW<>0 GROUP BY bedrijf
	INSERT @TB SELECT null,'31-12-'+@jaar,NULL,'MJVK Beheer BV'	, SUM(BTW),'BTW','BTW','',1172 
	FROM b17.boeking R WHERE jaar = @jaar AND boeknr>2000 AND BTW<>0 GROUP BY bedrijf
	...
	SELECT @BoekingID=boekingID 
	FROM gb.boeking WHERE gb.boeking.omschrijving=@jaar+' BTW schuld Eenheid'
	--SELECT @BTW = -ROUND(SUM(BTW),0) FROM b17.boeking R WHERE jaar = @jaar AND boeknr>2000 AND BTW<>0
	SELECT @BTW = -SUM(BTW) 
	...
	SELECT @Betaald=-SUM(Bedrag) 
	FROM b17.boekingbank WHERE id=@BoekingID AND jaar<=@jaar 

	--SELECT @BTW,@Betaald,@BTW-@Betaald

	INSERT @TB SELECT id,datum,NULL,bedrijf,-Bedrag,boekOms,Omschrijving,Relatie,BoekNr 
	FROM b17.boeking R WHERE jaar = @jaar AND BoekNr = 1256 
	...
	IF @BTW-@Betaald>0 
		INSERT @TB SELECT id,datum,NULL,bedrijf,@BTW,boekOms,Omschrijving,Relatie,1252 
		FROM b17.boeking R WHERE jaar = @jaar AND BoekNr =1252 
	ELSE
	BEGIN
		INSERT @TB SELECT id,datum,NULL,bedrijf,@Betaald,boekOms,Omschrijving,Relatie,1252 
		FROM b17.boeking R WHERE jaar = @jaar AND BoekNr =1252 
		INSERT @TB SELECT id,datum,NULL,bedrijf,-(@BTW+@Betaald),boekOms,Omschrijving,Relatie,1152 
		FROM b17.boeking R WHERE jaar = @jaar AND BoekNr =1252 
	END

	-- Boek alle kas betalingen op kas
	INSERT @TB SELECT id,datum,NULL,bedrijf,Bedrag,boekOms,'KAS '+Omschrijving,Relatie,1172 
	FROM b17.boeking R WHERE jaar = @jaar AND kas IN ('k','c') 

	-- Boek alls kas betalingen op BALANS post af van balans post
	INSERT @TB SELECT id,datum,NULL,bedrijf,Excl,boekOms,'KAS '+Omschrijving,Relatie,boekNr 
	FROM b17.boeking R WHERE jaar = @jaar AND kas IN ('k','c') AND boeknr<2000 
	INSERT @TB 
	...
	FROM b17.boeking B
	LEFT OUTER JOIN (SELECT id,SUM(bedrag)bedrag 
	FROM b17.boekingbank WHERE jaar<=@jaar GROUP BY id)BB ON BB.id=B.id 
	...
	--SELECT B.bankid id,B.datum,NULL,BD.bedrijf,CASE WHEN B.AfBij='Af' THEN -B.AfBijBedrag ELSE B.AfBijBedrag END bedrag,'Bank',Mededelingen omschrijving,'',1171
	--FROM dbo.bank B
	--INNER JOIN gb.bedrijf BD ON BD.rek_nr=B.rekening
	...
	SELECT @jaar,boeknr,RTRIM(bedrijf)bedrijf,sum(bedrag)
	FROM @TB 
	...
	SELECT keyID,datum,RTRIM(bedrijf)bedrijf,bedrag,boekNaam boekOms,boekNaam+ISNULL(': '+Relatie,'')+ISNULL(': '+omschrijving,'')omschrijving,Relatie,boekNr
	FROM @TB 
