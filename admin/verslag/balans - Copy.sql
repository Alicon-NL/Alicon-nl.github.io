...
AS
	...
	-- TEVEEL KAS AFLOSSEN SCHULD DGA
	INSERT gb.regel(BoekingID, BedrijfID, Datum, BoekNR, Bedrag)
	SELECT 0, BedrijfID, '31-12-'+@jaar Datum, 1254, - CEILING(SUM(RT.Bedrag)/1000)*1000 Bedrag
	FROM gb.regel RT 
	...
	INSERT gb.regel(BoekingID, BedrijfID, Datum, BoekNR, Bedrag)
	SELECT 0, BedrijfID, '01-01-'+@jaar Datum, 1254, - FLOOR(SUM(RT.Bedrag)/1000)*1000 Bedrag
	FROM gb.regel RT 
	WHERE DATEPART(YEAR,RT.Datum)=@jaar AND RT.BoekNR = 1172
	GROUP BY RT.BedrijfID, RT.BoekNR
