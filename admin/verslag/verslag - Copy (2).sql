...
ALTER VIEW b17.boekingbanksum
AS
	SELECT 
		B.bankId,CASE WHEN B.AfBij='Af' THEN -B.AfBijBedrag ELSE B.AfBijBedrag END bedrag,BB.boekBedrag,B.datum,DATEPART(year,B.datum) jaar,BD.bedrijf,Mededelingen
	FROM
		dbo.bank B 
		LEFT OUTER JOIN (SELECT bankID,SUM(bedrag)boekbedrag FROM gb.boekingbank GROUP BY bankID) BB ON BB.bankId=B.bankid
		INNER JOIN gb.bedrijf BD ON BD.rek_nr=B.rekening
GO
--SELECT * FROM b17.boekingbanksum where jaar=2018 AND bedrijf='Alicon Projects BV' ORDER BY datum
GO
