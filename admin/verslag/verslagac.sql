...
AS
	...
	--and	bd.bedrijf IN ('Alicon Systems BV','Alicon Projects BV')
GO
...
AS
	...
	----INSERT ALLE BOEKINGEN UIT ALICONNECT
	--INSERT @TB 
	--SELECT I.keyID,CONVERT(DATE,ISNULL(I.startDt,GETDATE()))startDt,F.*,P.boekNr 
 --   FROM (SELECT * FROM aim.api.fieldspivot(2347321,1180)) X PIVOT(max(value) 
 --   FOR name in (Company,Bedrag,BTW,Kas,BoekNr,Omschrijving,Relatie)) F
 --   INNER JOIN aim.api.items I ON I.id=F.id AND I.startDt<='31-12-'+@jaar
	--LEFT OUTER JOIN b17.post P ON P.bedrijf=F.Company AND P.omschrijving=F.BoekNr

	INSERT @TB 
	SELECT null,'31-12-'+@jaar,NULL,bedrijf,-SUM(Excl)*0.2,'Vennootschap','Vennootschap','',3001
    FROM b17.boeking R 
	WHERE datum <= '31-12-'+@jaar AND boeknr>=2000
	GROUP BY bedrijf

	INSERT @TB SELECT null,datum,NULL,bedrijf,-Bedrag,'Vennootschap','Vennootschap',bedrijf,1256 FROM @TB WHERE boeknr=3001
	INSERT @TB SELECT null,datum,NULL,bedrijf,Bedrag,'Vennootschap','KAS BETAALD Vennootschap',bedrijf,1256 FROM @TB WHERE boeknr=3001
	INSERT @TB SELECT null,datum,NULL,bedrijf,Bedrag,'Vennootschap','KAS BETAALD Vennootschap',bedrijf,1172 FROM @TB WHERE boeknr=3001
	INSERT @TB SELECT null,datum,NULL,'MJVK Beheer BV',-Bedrag,'Vennootschap','KAS ONTVANGEN Vennootschap',bedrijf,1172 FROM @TB WHERE boeknr=3001
	
	UPDATE gb.boeking SET bedrag=B.Bedrag,excl=B.bedrag
	FROM ( SELECT SUM(-B.bedrag)Bedrag FROM @TB B WHERE B.boeknr=3001) B
	WHERE gb.boeking.omschrijving=@jaar+' VB schuld Eenheid'

	DECLARE @btw FLOAT
	SELECT @btw=SUM(-BTW) FROM b17.boeking WHERE BTW<>0 AND jaar = @jaar

	UPDATE gb.boeking SET bedrag=@btw,excl=@btw WHERE gb.boeking.omschrijving=@jaar+' Schuld omzetbelasting Eenheid'

	INSERT @TB SELECT null,'31-12-'+@jaar,null,Bedrijf,sum(-BTW),'BTW Omzet','BTW Omzet','',1252 FROM b17.boeking WHERE BTW>0 AND jaar = @jaar GROUP BY Bedrijf
	INSERT @TB SELECT null,'31-12-'+@jaar,null,Bedrijf,sum(-BTW),'BTW Omzet','BTW Omzet','',1172 FROM b17.boeking WHERE BTW>0 AND jaar = @jaar GROUP BY Bedrijf
	INSERT @TB SELECT null,'31-12-'+@jaar,null,Bedrijf,sum(BTW),'BTW Voorbelast','BTW Voorbelast','',1152 FROM b17.boeking WHERE BTW<0 AND jaar = @jaar GROUP BY Bedrijf
	INSERT @TB SELECT null,'31-12-'+@jaar,null,Bedrijf,sum(-BTW),'BTW Voorbelast','BTW Voorbelast','',1172 FROM b17.boeking WHERE BTW<0 AND jaar = @jaar GROUP BY Bedrijf


	-- INSERT ALLE BOEKINGEN W&V
	INSERT @TB 
	SELECT id,datum,NULL,bedrijf,Excl,boekOms,Omschrijving,Relatie,boekNr 
    FROM b17.boeking R WHERE jaar <= @jaar AND boeknr>2000

	-- INSERT ALLE BOEKINGEN W&V KAS
	INSERT @TB 
	SELECT id,datum,NULL,bedrijf,BTW,boekOms,Omschrijving,Relatie,boekNr 
    FROM b17.boeking R WHERE jaar <= @jaar AND boeknr>2000 

	-- INSERT ALLE BOEKINGEN W&V KAS
	INSERT @TB 
	SELECT id,datum,NULL,bedrijf,-Bedrag,boekOms,Omschrijving,Relatie,boekNr 
    FROM b17.boeking R WHERE jaar <= @jaar AND boeknr>2000 AND kas IN('k','c')

	---- INSERT ALLE BOEKINGEN W&V KAS
	--INSERT @TB 
	--SELECT id,datum,NULL,bedrijf,Bedrag,boekOms,Omschrijving,Relatie,1172 
 --   FROM b17.boeking R WHERE jaar <= @jaar AND boeknr>2000 AND kas IN('k','c')

	--INSERT @TB 
	--SELECT keyid,datum,id,Bedrijf,BTW,'BTW Omzet',Omschrijving,Relatie,1252
	--FROM b17.boeking R WHERE jaar <= @jaar AND boeknr>2000 AND BTW>0

	--INSERT @TB 
	--SELECT keyid,datum,id,Bedrijf,-BTW,'BTW Voorbelast',Omschrijving,Relatie,1152
	--FROM @TB WHERE BTW<0
	...
	---- INSERT ALLE BOEKINGEN UIT ABIS < 2000
	--INSERT @TB 
	--SELECT id,datum,NULL,bedrijf,Excl,BTW,CASE WHEN kas IN('k','c') THEN bedrag ELSE 0 END,boekOms,Omschrijving,Relatie,boekNr 
 --   FROM b17.boeking R WHERE jaar <= @jaar AND boeknr<2000

	--INSERT @TB 
	--SELECT keyid,datum,id,Bedrijf,Kas,null,null,'Kas','Kas: '+Omschrijving,Relatie,1172
	--FROM @TB WHERE Kas<>0
	...
	INSERT @TB 
	SELECT null,'31-12-'+@jaar,NULL,bedrijf,SUM(Excl)*0.8,'Winst','Winstreserve','',1224
    FROM b17.boeking R 
	WHERE jaar <= @jaar AND boeknr>=2000
	GROUP BY bedrijf


	---- INSERT ALLE BANK BOEKINGEN
	--INSERT @TB 
	--SELECT BB.id,datum,null,Bedrijf,bedrag,0,0,'Bank',Mededelingen,'',1171 
	--FROM b17.boekingbank BB
	--WHERE datum <= '31-12-'+@jaar AND BB.id IN (SELECT keyid FROM @TB) --AND B.bedrijf=BB.bedrijf
	...
	--INSERT @TB 
	--SELECT B.id,BB.datum,NULL,B.bedrijf,-BB.bedrag,0,0,'Bank','BANK'+omschrijving,'',boeknr
	--FROM b17.boeking B
	...
	INSERT @TB 
	SELECT B.id,BB.datum,NULL,B.bedrijf,BB.bedrag,'Bank',omschrijving,'',1172
	FROM b17.boeking B
	INNER JOIN b17.boekingbank BB ON BB.id = B.id AND BB.jaar=@jaar AND B.bedrijf<>BB.bedrijf
	...
	INSERT @TB 
	SELECT B.id,BB.datum,NULL,BB.bedrijf,-BB.bedrag,'Bank',omschrijving,'',1172
	FROM b17.boeking B
	INNER JOIN b17.boekingbank BB ON BB.id = B.id AND BB.jaar=@jaar AND B.bedrijf<>BB.bedrijf
	...
	INSERT @TB 
	SELECT B.id,B.datum,NULL,B.bedrijf,B.bedrag-ISNULL(BB.bedrag,0) AS openstaand,'Bank',omschrijving,'',1151
	FROM b17.boeking B
	...
	INSERT @TB 
	--DEBITEUREN CREDITEUREN
	SELECT B.id,B.datum,NULL,B.bedrijf,-(B.bedrag-ISNULL(BB.bedrag,0)) AS openstaand,'Bank',omschrijving,'',1251
	FROM b17.boeking B
	LEFT OUTER JOIN (SELECT id,SUM(bedrag)bedrag FROM b17.boekingbank WHERE jaar<=@jaar GROUP BY id)BB ON BB.id=B.id
	WHERE B.bedrag<0 AND B.bedrag-ISNULL(BB.bedrag,0)<>0 AND B.jaar<=@jaar AND boeknr>=2000 AND B.kas NOT IN ('k','c')
